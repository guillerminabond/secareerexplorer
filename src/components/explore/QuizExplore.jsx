...
import { SEARCH_PATTERNS, parseSearch, removeCondition, searchOrgs } from "@/lib/searchUtils";
          ...
          <div className="flex flex-wrap items-center gap-2 mb-3">
            ...
            {search.conditions.map((c, i) => (
              <span key={i} className="inline-flex items-center gap-1 px-2.5 py-1 bg-crimson/10 text-crimson text-xs font-medium rounded-full border border-crimson/20">
                {c.label}
                <button onClick={() => setSearchQuery(removeCondition(searchQuery, c.label))} className="ml-0.5 hover:text-crimson/70"><X className="w-3 h-3" /></button>
              </span>
            ))}
            {parseSearch(searchQuery).keyword && (
              <span className="text-xs text-gray-400">+ keyword: "{parseSearch(searchQuery).keyword}"</span>
            )}
          </div>
        )}
