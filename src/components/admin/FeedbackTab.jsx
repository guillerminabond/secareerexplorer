...
export default function FeedbackTab({ feedback: initialFeedback, onReload }) {
  ...
  const saveComment = async (id) => {
    ...
    try {
      ...
      patch(id, { admin_comment: comment });
              ...
              {filtered.map(f => {
                ...
                return (
                  <React.Fragment key={f.id}>
                    <tr
                      className={`hover:bg-gray-50 cursor-pointer transition-colors ${f.starred && !f.archived ? "bg-amber-50/30" : ""}`}
